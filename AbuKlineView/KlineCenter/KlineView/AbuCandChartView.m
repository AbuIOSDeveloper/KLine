...
- (void)initCurrentDisplayModels
{
    ...
    if (currentStartIndex < count)
    {
        ...
        {
            AbuChartCandleModel *model = self.dataArray[i];
...
- (void)calcuteMaxAndMinValue
{
    ...
    {
        AbuChartCandleModel * entity = [self.currentDisplayArray objectAtIndex:i];
...
- (void)calcuteMaLinePostion
{
    ...
    {
        AbuKilneData *lineData = [maLines objectAtIndex:i];
        NSMutableArray *array = [NSMutableArray array];
        for (NSInteger j = 0;j <lineData.data.count; j++)
        {
            AbuKlineUniti *until = lineData.data[j];
            
            CGFloat xPosition = self.leftPostion + ((self.candleWidth  + self.candleSpace) * j) + self.candleWidth/2;
            CGFloat yPosition = ((self.maxY - until.value) * self.scaleY) + topDistance;
            //转换模型
...
- (void)initModelPositoin
{
    ...
    for (NSInteger i = 0 ; i < self.currentDisplayArray.count; i++)
    {
        ...
        AbuPostionModel *positionModel = [AbuPostionModel itemWithOpen:CGPointMake(left, open) close:CGPointMake(left, close) high:CGPointMake(left, high) low:CGPointMake(left,low) date:entity.date];
...
- (void)drawMALineLayer
{
    ...
    AbuKlineModel *ma5Model = self.maPostionArray[0][0];
    AbuKlineModel *ma10Model = self.maPostionArray[1][0];
    AbuKlineModel *ma25Model = self.maPostionArray[2][0];
    ...
    self.ma20LineLayer.strokeColor = ma25Model.lineColor.CGColor;
...
- (void)refreshKLineView:(AbuChartCandleModel *)model
{
      ...
      AbuChartCandleModel * Model = self.dataArray.lastObject;
      NSUInteger index = self.dataArray.count - 1;
        if ([Model.date isEqualToString:model.date]) {
...
- (void)drawKLine
{
    ...
    {
        AbuChartCandleModel *lastModel = self.currentDisplayArray.lastObject;
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(refreshCurrentPrice:candleModel:)]) {
        ...
        AbuChartCandleModel *candelModel = self.currentDisplayArray.lastObject;
...
- (void)initMacdMaxAndMinValue
{
    ...
    for (NSInteger i = 0;i<self.currentDisplayArray.count;i++)
    {
        AbuChartCandleModel *macdData = [self.currentDisplayArray objectAtIndex:i];
    ...
    NSDictionary *resultDic = [[AbuCalculate sharedInstance] calculateMaxAndMinValueWithDataArr:@[DEAArray,MACDArray,DIFArray]];
...
- (void)initMacdLinePostions
{
    ...
    for (int i = 0; i < self.currentDisplayArray.count; ++i) {
        AbuChartCandleModel *lineData = [self.currentDisplayArray objectAtIndex:i];
...
- (void)initKdjMaxAndMinValue
{
    ...
    NSMutableArray * kdjDataArray = computeKDJData(self.currentDisplayArray);
    for (AbuKilneData *lineData in kdjDataArray)
    {
        NSArray * array = lineData.data;
        for (NSInteger j = 0;j<array.count;j++)
        {
            AbuKlineUniti *until = array[j];
            self.indicatorMinY = self.indicatorMinY < until.value ? self.indicatorMinY : until.value;
            self.indicatorMaxY = self.indicatorMaxY > until.value ? self.indicatorMaxY : until.value;
        }
    }
...
- (void)initKdjLinePostions
{
    ...
    NSMutableArray * kdjDataArray = computeKDJData(self.currentDisplayArray);
    NSMutableArray *dataArray = (NSMutableArray*)[[kdjDataArray reverseObjectEnumerator] allObjects];
    for (AbuKilneData *lineData in dataArray)
    {
        NSArray *array = lineData.data;
        ...
        for (NSInteger j = array.count - 1;j>= 0;j--)
        {
            AbuKlineUniti *until = array[j];
            CGFloat xPosition = self.leftPostion + ((self.candleWidth  + self.candleSpace) * j) + self.candleWidth/2;
            CGFloat yPosition = ((self.indicatorMaxY - until.value) *self.indicatorScaleY)  + self.scrollView.height * orignIndicatorOrignY + midDistance;
            AbuKlineModel *model = [AbuKlineModel  initxPositon:xPosition yPosition:yPosition color:lineData.color];
...
- (void)initWrPostionModels
{
    ...
    NSMutableArray * macdDataArray = computeWRData(self.currentDisplayArray, 10);
    
    AbuKilneData * lineData = macdDataArray.firstObject;
    for (int i = 0; i < lineData.data.count; ++i) {
        AbuKlineUniti * until = lineData.data[i];
